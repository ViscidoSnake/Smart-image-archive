...
class img_obj():
    ...
    def extract_faces_features(self, l_type):
        ...
        for face in b:
            ...
            # considero anche il det score, parametro controverso e difficile da interpretare, viene usato nel calcolo del coefficiente e questo è abbastanza sbagliato perchè il parametro quantifica la confidenza con cui il motore di detection vede il volto nell'immagine, tuttavia alti valori di questo parametro garantiscono buone caratteristiche del volto e questo torna utile perche attualmente i parametri calcolati sotto sono abbastanza fragili...essendo un indicatore improprio nella media pesata il peso è basso 
            det_score = copy.deepcopy(face.det_score)
            weighted_det_score = 0.10 * det_score
            ...
            weighted_general_centering = 0.10 * n_general_centering
            ...
            weighted_mouth_centering = 0.10 * n_mouth_centering
            ...
            weighted_yaw = 0.05 * n_yaw # peso molto piccolo, variabile molto poco affidabile

            # stima della rotazione verticale
            roll = np.arctan2(re[1]-le[1], re[0]-le[0])
            roll_deg = np.degrees(roll)
            n_roll = 1 - np.clip((roll_deg - 0.0)/(0.20 - 0.0), 0.0, 1.0) 
            weighted_roll = 0.05 * n_roll # peso molto piccolo, variabile molto poco affidabile
            ...
            quality_score = 0.0
            if min(size) > 70: 
                quality_score = weighted_general_centering + weighted_mouth_centering + weighted_roll + weighted_yaw + weighted_contrast_range + weighted_sat + weighted_blur + weighted_det_score
            ...
            features = {
                ...
                "kps_coord" : kps_coord,
                "size" : size,
                "det_score" : (det_score, det_score), # det_score è giù normalizzato, mando la tupla come standard
